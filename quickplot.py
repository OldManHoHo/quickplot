...
class plot_improved(QtGui.QWidget):
    def __init__(self, in_plot, win_size_x = 500, win_size_y = 400):

        super(plot_improved, self).__init__()
...
class plot(QtGui.QWidget):
    ...
    def __init__(self, in_plot, win_size_x = 500, win_size_y = 400):
        ...
        self.window_surface = QtGui.QGraphicsScene()
        self.bg_brush = QtGui.QBrush(QtGui.QColor(200,200,200))
        self.ext_surface = QtGui.QGraphicsScene(0,0,win_size_x,win_size_y)
        self.plot_surface = QtGui.QGraphicsScene(0,0,win_size_x,win_size_y)
        self.initUI()
    ...
    def initUI(self):
        ...
        self.view = QtGui.QGraphicsView(self.ext_surface, self)
        self.view.setAcceptDrops(True)
        self.plot_view = view_plot(self.plot_surface, self.view)
        self.plot_view.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        self.plot_view.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        self.view.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        self.view.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
    ...
    def draw_plot(self, size_x = 500, size_y = 400):
        ...
        #self.plot_surface = QtGui.QGraphicsScene(0,0,plot_width-4,plot_height-4)
        self.plot_surface.setSceneRect(0,0,plot_width-2,plot_height-2)
        self.plot_surface.clear()
        self.plot_surface.setBackgroundBrush(QtGui.QBrush(QtGui.QColor(255,255,255)))  
        ...
        #textRect.centerx = self.ext_surface.get_rect().centerx
        #textRect.centery = self.ext_surface.get_rect().centery - int(size_y*0.9)/2
        title_text = self.ext_surface.addSimpleText("X vs. Y" ,self.title_font)
        title_pos_y = size_y/2 - title_text.boundingRect().height()/2 - int(size_y*0.9)/2
        if title_pos_y < 0:
            title_pos_y = 0
        title_text.setPos(size_x/2 - title_text.boundingRect().width()/2, title_pos_y)
        ...
        for tick in range(int(x_min/x_div),int(x_max/x_div)+2):
            ...
            if abs(tick*x_div - x_max) < (x_max - x_min)*1.0/plot_width:
                ...
                tick_position = plot_width*(tick-(x_min/x_div))/(x_max/x_div-x_min/x_div)
            self.plot_surface.addLine(tick_position, plot_height - 3, tick_position,plot_height - x_tick_size)
            tick_text = self.ext_surface.addSimpleText(str(tick*x_div) ,self.tick_font)
            tick_pos_y = plot_top + plot_height + 5
            tick_pos_x = tick_position + plot_left - tick_text.boundingRect().width()/2
        ...
        for tick in range(int(y_min/y_div),int(y_max/y_div)+2):
            ...
            if abs(tick*y_div - y_max) < (y_max - y_min)*1.0/plot_height:
                ...
                tick_position = plot_height*(tick-(y_min/y_div))/(y_max/y_div-y_min/y_div)
            self.plot_surface.addLine(-1, plot_height - tick_position, y_tick_size, plot_height - tick_position)
            tick_text = self.ext_surface.addSimpleText(str(tick*y_div) ,self.tick_font)
            tick_pos_x = plot_left - 5 - tick_text.boundingRect().width()
            tick_pos_y = plot_height - tick_position + plot_top - tick_text.boundingRect().height()/2
        ...
        for d in self.plots:
            ...
            if d.plot_type == "scatter":
                for i in range(0,len(d.data_x)):
                    #new_pg = QtGui.QPolygonF(6)
                    #self.plot_surface.addPolygon(new_pg)
                    #new_pg.translate(int(plot_width*(d.data_x[i]-x_min)/(x_max-x_min)), int(plot_height*(-d.data_y[i]-y_min)/(y_max-y_min)))
                    self.plot_surface.addRect(int(plot_width*(d.data_x[i]-x_min)/(x_max-x_min)) - d.point_size/2, plot_height - int(plot_height*(d.data_y[i]-y_min)/(y_max-y_min)) - d.point_size/2,d.point_size,d.point_size, brush=QtGui.QBrush(QtGui.QColor(color[0],color[1],color[2])))
                    #pygame.draw.circle(self.plot_surface, color, (int(plot_width*(d.data_x[i]-x_min)/(x_max-x_min)), int(plot_height*(-d.data_y[i]-y_min)/(y_max-y_min))), 2)
            elif d.plot_type == "line":
                line_list = [[
                                plot_width*(d.data_x[i]-x_min)/(x_max-x_min),
                                plot_height - plot_height*(d.data_y[i]-y_min)/(y_max-y_min)
                            ] for i in range(len(d.data_x))]
                #pygame.draw.aalines(self.plot_surface, color, False, line_list,2)
                for i in range(0,len(line_list)-1):
                    plot_pen = QtGui.QPen()
                    plot_pen.setWidth(d.line_width)
                    plot_pen.setColor(QtGui.QColor(color[0],color[1],color[2]))
                    self.plot_surface.addLine(line_list[i][0],line_list[i][1],line_list[i+1][0],line_list[i+1][1],plot_pen)
            d.data_lock.release()
        ...
        if self.plot_view.mouse_press_active_left:
            box_pen = QtGui.QPen()
            box_pen.setStyle(QtCore.Qt.DashLine)
            self.plot_surface.addRect(self.plot_view.mouse_press_point.x(),
                                      self.plot_view.mouse_press_point.y(),
                                      self.plot_view.mouse_current_point.x() - self.plot_view.mouse_press_point.x(), 
                                      self.plot_view.mouse_current_point.y() - self.plot_view.mouse_press_point.y(),
                                      box_pen)
        if self.plot_view.process_release():
            self.auto_axis_x = False
            self.auto_axis_y = False
        ...
        if self.plot_rect.collidepoint(m_pos):
            pygame.draw.rect(self.ext_surface, WHITE, pygame.Rect(m_pos[0]+10,m_pos[1]+15,30,12),0) 
            pygame.draw.rect(self.ext_surface, BLACK, pygame.Rect(m_pos[0]+10,m_pos[1]+15,30,12),1)
            point_text = self.point_font.render("test", True, BLACK)
            point_text_rect = point_text.get_rect()
            point_text_rect.left = m_pos[0]+10
            point_text_rect.top = m_pos[1]+15
            self.ext_surface.blit(point_text, point_text_rect)
            
            if pygame.mouse.get_pressed()[0]:
                if self.axis_menu == False:
                    self.axis_menu = True
                    #self.add_textbox("x axis",150,30,5,5,str(x_div))
                else:
                    self.axis_menu = False
...
def main():
    ...
    ex = plot_improved(plot_data("line",[x/10.0 for x in range(0,100,2)],[5+math.sin(x/10.0) for x in range(0,100,2)]))
    ex.add_plot(plot_data("line",[x/10.0 for x in range(0,100,2)],[5+0.7071*math.cos(x/10.0) for x in range(0,100,2)]))
    ex2 = plot_improved(plot_data("line",[x/10.0 for x in range(0,100,2)],[math.sin(x/10.0) for x in range(0,100,2)]))
